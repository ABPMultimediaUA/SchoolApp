...
class Examen extends REST_Controller {
    ...
    public function examen_get()
    {
       ...
       /* $examen = [
        ...
        $id = $this->get('id');
		if(!isset($id))$id = $this->get("idExamen");
		...
		$nota = $this->get('nota');
		 ...
		 if(isset($alumnos)&&isset($id))
		{
			$alumnos = $this->examen_model->get_alumnosByExamen($id);
			if(!empty($alumnos))
            {
		...
		else if($idProfesor!=NULL && $idProfesor>0)
        {

            $idProfesor = (int) $idProfesor;

				$examenes = $this->examen_model->get_examenByProfesor($idProfesor);
        ...
        else if($idAlumno!=NULL && $idAlumno>=0)
        {
    ...
    public function examen_post()
    {
        ...
        if($res){$res=array('message'=>"Examen added to database", 'idExamen'=>$res );}
    ...
    public function examen_put()
    {
		...
		$put = $this->put();
		if(isset($put['idExamen']) && isset($put['idAlumno']) && isset($put['nota'] ))
		{
			$data = array("idAlumno"=>$put['idAlumno'], "idExamen"=>$put['idExamen'], "nota"=>$put['nota']);
			$res = $this->examen_model->put_nota($data);
			if($res)
			{
				$this->response([
                    'status' => TRUE,
                    'message' => "Nota actualizada a ".$put["nota"]
