...
class Profesor extends REST_Controller {
    ...
    public function profesor_get()
    {
       ...
       /* $profesor = [
		...
		$centro = $this->get('idCentro');


        // If the id parameter doesn't exist return all the profesor

		if(isset($centro) && $centro != NULL)
		{
			 $profesores = $this->profesor_model->get_profesorByCentro($centro);
			 if($profesores)
			 {
					 $this->response($profesores, REST_Controller::HTTP_OK);
			}
			 else
            {
        ...
        else if ($id === NULL && $user === NULL && $centro === NULL)
        {
    ...
    public function profesor_post()
    {
                ...
                $res = $this->profesor_model->post_profesor($this->post());
