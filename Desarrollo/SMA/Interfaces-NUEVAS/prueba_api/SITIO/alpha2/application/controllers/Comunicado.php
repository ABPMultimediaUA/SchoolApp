...
class Comunicado extends REST_Controller {
    ...
    public function comunicado_get()
    {
       ...
       /* $comunicado = [
		...
		$autorizacion = $this->get('autorizacion');
		$alumnosaut = $this->get('alumnosaut');
		$firmadas = $this->get('firmadas');
		$nofirmadas = $this->get('nofirmadas');
		$tipo = $this->get('tipo');
		$anunciosgestor = $this->get('anunciosGestor');
		$idCentro = $this->get('idCentro');

		if($anunciosgestor != NULL && $anunciosgestor && $idCentro != NULL){$res = $this->comunicado_model->get_anunciosGestor($idCentro);
		if(!empty($res))
        ...
        else if ($id === NULL && $idAlumno === NULL && $userAlumno === NULL && $idProfesor === NULL && $anunciosgestor === NULL )
        {
    ...
    public function comunicado_delete()
    {
		...
		if(($this->get('idAnuncioGestor'))!=NULL){
		$id = $this->get('idAnuncioGestor');
		$message =  $this->comunicado_model->delete_anuncioGestor($id);
