...
function initShaders() {
    ...
    shaderProgram.uLightAmbient = gl.getUniformLocation(shaderProgram, "uLightAmbient"); //Componente ambiente de la luz
    shaderProgram.uLightDiffuse = gl.getUniformLocation(shaderProgram, "uLightDiffuse"); //Componente difusa de la luz
    shaderProgram.uLightSpecular = gl.getUniformLocation(shaderProgram, "uLightSpecular"); //Componente especular de la luz
    shaderProgram.uKd = gl.getUniformLocation(shaderProgram, "uKd"); //Componente difusa del material
    shaderProgram.uKa = gl.getUniformLocation(shaderProgram, "uKa"); //Componente difusa del material
    ...
    shaderProgram.uParticles = gl.getUniformLocation(shaderProgram, "uParticles");
    //shaderProgram.uParticleSize = gl.getUniformLocation(shaderProgram, "uParticleSize");
}
...
/** actualiza las matrices de vista, de proyeccion y de normales**/
...
function drawScene() {
    requestAnimFrame(drawScene); //se ejecuta esta función 60fps
    handleKeys();
    motor.draw();
    animate();
}
...
function webGLStart() {
    ...
    /*** Creación del gestor de la escena ***/
    gestor = new TGestorRecursos();
    /*** Creación de la escena ***/
    motor = new TMotorTAG();
    
    motor.crearParticulas(400);
    ...
    /*var rotMallas = motor.crearTransform();
    rotMallas.rotar(1,0,0,90);*/
    ...
    var malla2 = motor.crearMallaConMat('mundo_divided.obj','mundo.mtl');
	var malla = motor.crearMalla('comunidades.obj');
    var malla3 = motor.crearMallaConText('capsule.obj','app/graphics_engine/Modelos/capsule0.jpg');

    //var malla = motor.crearMallaConText('mundotextura.obj','app/graphics_engine/Modelos/land_shallow_topo_8192.tif');
...
function fovyCam(fovy){
    var cam = motor.getCamaraActiva().nodo.getEntidad();
    cam.fovy = fovy;
}
...
function updateLight(){
    ...
    var luz = motor.getLuzActiva().nodo.getEntidad();
