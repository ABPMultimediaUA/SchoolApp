...
class mensaje extends REST_Controller {
    ...
    public function mensaje_get()
    {
       ...
       /* $mensaje = [
		...
		$leidos = $this->get('leidos');
		 ...
		 if($idProfesor != NULL && $idProfesor>=0 && $idAlumno == NULL && $idPadre == NULL)
        {
			if($leidos!=NULL && $leidos){
            $mensajes = $this->mensaje_model->get_leidosByProfesor($idProfesor);
            if (!empty($mensajes))
            {
		...
		else if($noleidos!=NULL && $noleidos){
            $mensajes = $this->mensaje_model->get_noLeidosByProfesor($idProfesor);
            if (!empty($mensajes))
            {
                $this->response($mensajes, REST_Controller::HTTP_OK); // OK (200) being the HTTP response code
            }
			...
			$mensajes = $this->mensaje_model->get_mensajesByProfesor($idProfesor);
			 if (!empty($mensajes))
            {
                $this->response($mensajes, REST_Controller::HTTP_OK); // OK (200) being the HTTP response code
            }
        ...
        else if($idProfesor != NULL && $idProfesor>=0 && $idAlumno != NULL && $idAlumno >= 0)
        {
		...
		else if($idProfesor == NULL  && $idPadre != NULL && $idPadre >= 0)
        {
				...
				{
					$mensajesrecibidos = [];
					$mensajes = $this->mensaje_model->get_recibidosByPadre($idPadre);
					if (!empty($mensajes))
					{
						...
						foreach($mensajes as $mensaje)
						{
							$conversacion = FALSE;
							$i=0;
							foreach($mensaje["MensajesProfesor"] as $elemento)
							{
								if(isset($elemento["leido"])&&!$elemento["leido"])
								{
									...
									$idprofe = $elemento['idProfesor'];
									$mensajes1 =  $this->mensaje_model->get_mensajesByPadre2($idPadre, $idprofe);
									$conversacion = TRUE;
								}
								$i++;
							}
				...
				{
					$mensajesrecibidos = [];
					$mensajes = $this->mensaje_model->get_recibidosByPadre($idPadres);
					if (!empty($mensajes))
					{
					...
					$mensajes = $this->mensaje_model->get_mensajesByPadre($idPadre);
				}
				if (!empty($mensajes))
				{
                $this->response($mensajes, REST_Controller::HTTP_OK); // OK (200) being the HTTP response code
		...
		else if($idProfesor == NULL  && $idAlumno != NULL && $idAlumno >= 0)
        {
						...
						foreach($mensajes as $mensaje)
						{
							...
							foreach($mensaje["MensajesProfesor"] as $elemento)
							{
								if(isset($elemento["leido"])&&!$elemento["leido"])
								{
									...
									//array_push($mensajesrecibidos, $elemento);
									$idprofe = $elemento['idProfesor'];
									$mensajes1 =  $this->mensaje_model->get_mensajesByAlumno2($idAlumno, $idprofe);
									$conversacion = TRUE;
								}
								$i++;
							}
    ...
    public function mensaje_post()
    {
		...
		$flujo = NULL;
		$res = FALSE;
		if(isset($emisor) && $emisor!=NULL && isset($receptor) && $receptor!=NULL)
		{
